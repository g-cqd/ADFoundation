...
/// --strict` over a target's Swift sources as a prebuild step, so a non-zero exit fails the
/// build. Attached to the library targets only when `ADF_DEV` is set (see Package.swift), so it
/// never runs for packages that merely depend on ADFoundation.
@main
